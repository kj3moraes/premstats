...
def create_match(match: Match, session: Session = Depends(get_db)):
    session.add(match)
    session.commit()
...
def read_matches(skip: int = 0, limit: int = 100, session: Session = Depends(get_db)):
    matches = session.exec(select(Match).offset(skip).limit(limit)).all()
    return matches


@router.get("/get/{match_id}", response_model=Match)
def read_match(match_id: int, session: Session = Depends(get_db)):
...
def update_match(match_id: int, match: Match, session: Session = Depends(get_db)):
